...
def token_obrigatorio(f):
    ...
    def decorated(*args, **kwargs):
        token = None
        #verificar se umtoken foi enviado
        if 'x-access-token' in request.headers:
            token = request.headers['x-access-token']
        if not token:
            return jsonify({'mensagem': 'O token não foi incluido!'}, 401)
        # se temos um token, validar acesso consultando o Banco de Dados:
        try:
            resultado = jwt.decode(token,app.config[
...
# Criar um Novo Post - POST http://localhost:5000/postagem
...
def nova_postagem(autor):
    ...
    return jsonify({'mensagem': 'postagem criada com susseço!'})

# Editar uma Postagem Existente - PUT http://localhost:5000/postagem/1
...
def alterar_postagem(autor, id_postagem):
    ...
    db.session.commit()
    return jsonify({'mensagem': 'Postagem alterda com susseço!'})

# Excluindo Uma Postagem - DELETE http://localhost:5000/postagem/1
...
def excluir_postagem(autor, id_postagem):
    ...
    if not postagem_a_ser_excluida:
        return jsonify({'mensagem': 
                        'Não foi encontrado postagem com este ID:'})
    db.session.delete(postagem_a_ser_excluida)
    db.session.commit()

    return jsonify({'mensagem': 'Postagem excluída com susseço!'})
...
def novo_autor(autor):
    ...
    return jsonify({'mensagem': 'Usuário criado com susseço'}, 200)
...
def alterar_autor(autor, id_autor):
    ...
    db.session.commit()
    return jsonify({'mensagem': 'Usuário alterado com susseço!'})
...
def excluir_autor(autor, id_autor):
    ...
    return jsonify({'mensagem': 'Autor excluido com susseço'})
